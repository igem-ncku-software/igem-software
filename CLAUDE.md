...
The firmware lives in `firmware/capture_screen/` (ESP32 + AS7341 + SSD1306; DFRobot_AS7341, Adafruit SSD1306/GFX, ArduinoJson 7, WebSockets by Markus Sattler/Links2004, and `Preferences` for the bench k5/k4 calibration). It replaced `firmware/as7341/`, which survives only in git history. Its comments are in Chinese. It runs two workflows side by side: the bench workflow (Serial sample ID + button → a batch of `N_RUN` readings, each `N_AVG` dark/light pairs, CSV on Serial, stats on the OLED; nothing of it is sent to the backend) and the web workflow. It runs no HTTP server. Once Wi-Fi connects it keeps one outbound `WebSocketsClient` to `BACKEND_HOST` + `/api/hardware/device` and speaks the protocol above: status on connect, on every state change, and every 5 s; frames back to back while `liveWanted`; and `read` answered with one single-shot dark → light → dark measurement (integer raw counts) tagged with the `request_id`, or `error` `busy` / `sensor_offline`.
- Wi-Fi is non-blocking: `setup()` waits at most 10 s, then `serviceWifi()` in `loop()` starts the WebSocket once Wi-Fi is up, so the bench workflow works with no network.
- A batch blocks `loop()` for 15+ s, so `readAvg()` calls `pump()` between reads: `ws.loop()` plus the 5 s status heartbeat. Without the heartbeat the backend marks the device offline mid-batch.
- Serial Monitor (115200) prints `[wifi]` / `[backend]` connection events and a "still connecting" line every 10 s — the first place to look when the device doesn't appear online. Web reads print a `#` line so the CSV stays parseable.
...
- State is IDLE / LIVE / MEASURING (a bench batch and blank calibration are MEASURING too). LIVE follows `liveWanted` whenever no measurement is running, so a read or batch preempts streaming and streaming resumes afterwards on its own. A backend disconnect clears `liveWanted`, so the LED never stays on with nobody watching. Serial `l`/`d` refuse to touch the LED outside IDLE.
...
- `FIRMWARE_VERSION` (5.2.0) feeds the config fingerprint, so bumping it makes every existing curve stale — intended when the reading path changes.
- `LED_PIN` is 25 on this build, not the reference design's GPIO 2.
